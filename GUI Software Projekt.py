...
class input_simulationA:
    ...
    def create_frame(self):
        ...
        # setzt bei Aufruf die Cursorposition in bestimmten Eingabefeldern auf die letzte Position
        def motion(event):
        ...
        # motion wird bei den Eingaben, welche die Position des Eingabecursors verändern, aufgerufen
        fenster.bind('<Button-1>', motion)
        fenster.bind('<ButtonRelease-1>', motion)
        fenster.bind('<Left>', motion)
        fenster.bind('<Right>', motion)
        fenster.bind('<B1-Motion>', motion)
        ...
        def limitSizeName(*args):
            ...
            i = 0
            while(i<len(value)):
                if(value[i].lower() not in "abcdefghijktlmnopqrstuvwxyz _-1234567890"):
                    value=value[:i]+value[i+1:]
                else:
                    i+=1
...
frame_test.frame.pack(fill=X, expand=True, side=BOTTOM, anchor=S)
# In[5]:
...
def set_simulation(num):
    ...
    if(num > -1 and toolbox[0] != -1):
        data_list[toolbox[0]][4].frame.pack_forget()
    elif(num >-1 and toolbox[0] == -1):
        frame_test.frame.pack_forget()
    ...
    if(num > -1 and toolbox[0] != -1):
        ...
        data_list[toolbox[0]][4].frame.pack(fill=X, expand=True, side=BOTTOM, anchor=S)
...
infection = np.array([[1,1,1],[1,0,1],[1,1,1]])*0.6 # Nachbarschaft für die Infektion
recovery = np.array([0.3]) # Nachabrschaft für die Erholung
...
sir = simulation(start, 500, flow_m, flow_c)
sir.run()
plt.imshow(sir.get(100)) # Simulation zum Zeitschritt 100
plt.show()
#------------------------------------------------------------------------------------------------------------------------------------------
